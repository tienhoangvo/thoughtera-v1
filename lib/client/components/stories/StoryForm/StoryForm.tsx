import { Stack, Box, Button, Group, InputWrapper, LoadingOverlay, Switch, Textarea, TextInput, Text, Card } from "@mantine/core"
...
type StoryFormProps = {
  ...
  onSave: ( id: string, submitData: StorySubmitDataType, finishSaving?: () => void ) => void
}

const StoryForm = ({ story, onSave }: StoryFormProps) => {
  const getInitialValue = (field: 'content' | 'title' | 'excerpt' | 'published') => {
    if (story) {
      return story[field]
    }
  ...
  const [published, setPublished] = useState(getInitialValue('published') as boolean)
  const [saving, setSaving] = useState(false)
  
  const storySubmitDataRef = useRef<StorySubmitDataType>({})
  const disabled = title.trim().length === 0 || excerpt.trim().length === 0 || content.trim() === '<p><br></p>'
    ...
    <Card withBorder>
      ...
      <Stack spacing="lg">
        <Group position="apart">
          <Button onClick={handelBackClick} variant="subtle" leftIcon={<ArrowLeft />}>Back</Button>
          <Button disabled={disabled} onClick={handleSaveClick} variant="light" color="teal" rightIcon={<PaperPlaneRight />}>Save</Button>
        </Group>
        <TextInput
          variant="filled"
          label="Title"
          ...
          autoFocus
        />
        <InputWrapper size="md" label="Public" description="Publish your story so that people can see it or keep it as a draft now.">
          <Switch sx={{ '&>input[type="checkbox"]': { cursor: "pointer" } }} onLabel="ON" offLabel="OFF" size="lg" checked={published} onChange={handlePublicSwitchChange} />
        </InputWrapper>
        <Textarea
          label="Excerpt"
          variant="filled"
        ...
        <InputWrapper label="Content" size="md">
          <RichTextEditor value={content} onChange={handleContentChange} sx={{ minHeight: '500px' }} />
        </InputWrapper>
      </Stack>
    </Card>
  )
