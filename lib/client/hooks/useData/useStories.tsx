...
const fetcher = ( key: string ) => {
  ...
  }).then(res => res.json()).then(data => data.stories as StoryListType)
}
...
const useStories = ( { username } : useStoriesProps) => {
  ...
  const { data, setSize, error } = useSWRInfinite((pageIndex, previousPageData) => {
  ...
  return {
    stories: data ? data.reduce((prev, curr) => {
