...
export const createStory = ( { title, content, excerpt, published=false } : createStoryProps) => {
...
export const updateStory = (id: string | undefined, data: updateStoryDataType) => {
...
export const listStories = ( key: string ) => {
  ...
  }).then(res => res.json()).then(data => data.stories as ListStoryType)
}
...
export const getStoryDetails = ( slug : string) => {
  ...
  }).then(res => res.json()).then(data => data.story as StoryType)
}